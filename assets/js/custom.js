  ...
  function toggleMenu() {
    if (isSmallScreen()) {
      if (!isMenuOpen) {
        ...
        $('body').addClass("menuOpen"); // Added jQuery toggle class
        isMenuOpen = true;
      } else {
        menu.style.display = "none";
        body.classList.remove("menu");
        $('body').removeClass("menuOpen"); // Added jQuery remove class
        isMenuOpen = false;
      }
    }
  }
  ...
  var astrology = {
    ...
    loader: function () {
      $(window).on('load', function () {
        $(".as_loader, .as_spinner").delay(500).fadeOut("hide");
      });
    },
    toggle: function() {
    ...
    quantity: function() {
      $('.quantity .plus').on('click', function(){
        var num = parseInt($(this).closest('.quantity').find('input').val())+1;
